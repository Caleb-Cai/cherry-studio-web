...
import type { Chat, Message, ChatResponse } from '@/types/api'
...
export const useChatStore = defineStore('chat', () => {
  ...
  async function loadChats(page = 1, limit = 20) {
    ...
    try {
      const response: ChatResponse = await ApiService.chat.list({ page, limit })
  ...
  async function createChat(data: {
    ...
    try {
      const chat: Chat = await ApiService.chat.create(data)
      chats.value.unshift(chat)
      currentChat.value = chat
      messages.value = chat.messages || []
      
      // Join the chat room for real-time updates
      if (socketService.joinChat) {
  ...
  async function loadChat(chatId: string) {
    isLoading.value = true
    try {
      const chat: Chat = await ApiService.chat.get(chatId)
      currentChat.value = chat
      messages.value = chat.messages || []
  ...
  async function updateChat(chatId: string, data: { title: string }) {
    try {
      const updatedChat: Chat = await ApiService.chat.update(chatId, data)
  ...
  async function searchChats(query: string) {
    try {
      const response: ChatResponse = await ApiService.chat.search(query)
  ...
  function clearCurrentChat() {
    if (currentChat.value && socketService.leaveChat) {
  ...
  function initializeSocketListeners() {
    if (socketService.onMessage) socketService.onMessage(handleSocketMessage)
    if (socketService.onStreamStart) socketService.onStreamStart(handleStreamStart)
    if (socketService.onStreamChunk) socketService.onStreamChunk(handleStreamChunk)
    if (socketService.onStreamComplete) socketService.onStreamComplete(handleStreamComplete)
    if (socketService.onStreamError) socketService.onStreamError(handleStreamError)
  }
