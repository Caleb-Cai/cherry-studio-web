...
export const useModelStore = defineStore('model', () => {
  ...
  async function loadModels(provider?: string) {
    isLoading.value = true
    try {
      const response = await ApiService.model.list(provider)
      ...
      if (provider) {
        ...
        if (response.providers) {
          providers.value = response.providers.map((id: string) => ({
            id,
            name: id,
  ...
  async function loadModelInfo(provider: string, modelId: string) {
    try {
      const model = await ApiService.model.getInfo(provider, modelId)
  ...
  async function searchModels(query: string, filters?: {
    ...
    try {
      const response = await ApiService.model.search(query, filters)
