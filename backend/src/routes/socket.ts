...
export function setupSocketRoutes(io: Server) {
  ...
  io.on('connection', (socket: Socket) => {
    const authenticatedSocket = socket as SocketWithAuth;
    ...
    socket.on('join-chat', async (chatId: string) => {
      try {
        ...
        const chat = await prisma.chat.findFirst({
          where: { id: chatId, userId: authenticatedSocket.userId }
        });

        if (chat) {
          socket.join(`chat:${chatId}`);
          logger.info(`User ${authenticatedSocket.userId} joined chat ${chatId}`);
        }
      } catch (error) {
    ...
    socket.on('leave-chat', (chatId: string) => {
      ...
      logger.info(`User ${authenticatedSocket.userId} left chat ${chatId}`);
    });
    ...
    socket.on('file-uploaded', async (data: { fileId: string }) => {
      try {
        const file = await prisma.file.findFirst({
          where: { id: data.fileId, userId: authenticatedSocket.userId }
        });
  ...
  (io as any).sendToChat = (chatId: string, event: string, data: any) => {
